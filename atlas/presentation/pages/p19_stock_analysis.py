...
""", unsafe_allow_html=True)
    ...
    with r0[4]:
        # 單量 = 成交量 / 成交筆數（無筆數時顯示 N/A）
        st.markdown(
            metric_card("振幅", f"{(float(last['high']) - float(last['low'])):.2f}"),
            unsafe_allow_html=True,
        )
    with r0[5]:
    ...
    with r1c2:
        ...
        try:
            # 六維評分
            rsi_score = max(0, min(100, int(rsi_val))) if rsi_val == rsi_val else 50
            macd_hist = float(last.get("MACD_hist", 0))
            macd_hist = macd_hist if not pd.isna(macd_hist) else 0
            macd_score = max(0, min(100, 50 + int(macd_hist * 10)))

            vol_avg = float(df["volume"].iloc[-20:].mean()) if len(df) >= 20 else 1
            vol_score = max(0, min(100, int(vol / vol_avg * 50))) if vol_avg > 0 else 50

            # 趨勢分：MA8 vs MA21 距離
            ma8_v = float(df["MA8"].iloc[-1]) if "MA8" in df.columns else close_price
            ma21_v = float(df["MA21"].iloc[-1]) if "MA21" in df.columns else close_price
            ...
            # 動能分（近 5 日漲幅）
            if len(df) >= 5:
                ret_5d = (close_price / float(df["close"].iloc[-5]) - 1) * 100
                momentum_score = max(0, min(100, 50 + int(ret_5d * 5)))
            else:
                momentum_score = 50

            categories = ["趨勢", "動能", "量能", "技術(RSI)", "波動", "MACD"]
            values = [
            ...
            grade = (
                ...
                else "B" if overall >= 55 else "C" if overall >= 40 else "D"
            )
    ...
    with r1c3:
        ...
        try:
            ...
            st.markdown(
                f"<div style='text-align:center; font-size:48px;'>{light}</div>",
                unsafe_allow_html=True,
            )
            st.markdown(
    ...
    with r2c1:
        ...
        try:
            atr_for_fan = atr_val if atr_val > 0 else close_price * 0.02
            # 簡易機率估算：趨勢 + RSI
            if trend_score > 60:
    ...
    with r2c2:
        ...
        try:
            ...
            if not vp.empty:
                price_levels_list = vp["price_level"].astype(float).tolist()
                volumes_list = vp["volume"].astype(float).tolist()

                # 支撐壓力
                plc = get_price_level_calc()
                pl_result = plc.calculate(ohlcv_df, code=code)
                support = pl_result.supports[0] if pl_result.supports else None
                resist = pl_result.resistances[0] if pl_result.resistances else None
    ...
    with r3c1:
        ...
        try:
            ...
            dtr_result = dtr.analyze(code, ohlcv_df, fund_flow_data=flow_for_dtr)
            ...
            st.markdown(
                ...
                f"color:{risk_color};'>風險等級：{dtr_result.risk_level}</div>",
                unsafe_allow_html=True,
            )
            st.divider()
            st.markdown(
    ...
    with r3c2:
        ...
        try:
            ...
            fig = gauge_rings(metrics, columns=5, height=220)
            st.plotly_chart(fig, use_container_width=True)

            avg_health = int(sum(m["value"] for m in metrics) / len(metrics))
            health_label = (
                "健康" if avg_health >= 70 else "普通" if avg_health >= 50
                else "偏弱"
            )
            health_status = (
                "positive" if avg_health >= 70 else "neutral" if avg_health >= 50
                else "negative"
            )
            st.markdown(
                metric_card("綜合健康度", f"{avg_health} / 100 — {health_label}",
    ...
    with r3c3:
        ...
        try:
            ...
            st.markdown(
                ...
                f"<div style='text-align:center; font-size:18px; "
                f"color:{level_color};'>信心等級：{conf_result.level}</div>",
                unsafe_allow_html=True,
            )
            st.divider()
            for dim in conf_result.dimensions:
                dim_status = (
                    "positive" if dim.score >= 70 else "neutral" if dim.score >= 50
                    else "negative"
                )
                st.markdown(
                    f"**{dim.name}**：{dim.score} 分 — {dim.description}"
                )
        except Exception as e:
    ...
    with r4c1:
        ...
        try:
            ...
            fig = energy_bar(bull_pct, bear_pct, height=100)
            st.plotly_chart(fig, use_container_width=True)
        except Exception as e:
            st.info(f"多空能量載入中… ({e})")
