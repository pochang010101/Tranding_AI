"""測試 OTC（上櫃）股票支援 — _is_otc、yfinance ticker、TWSE MIS ex_ch。"""
...
class TestDataManagerOTC:
    def test_is_otc_known_codes(self):
        from atlas.infrastructure.data_manager import _is_otc

        known_otc = ["5269", "6488", "6669", "3293", "8069", "6147", "3529", "6770", "8454", "5871"]
        for code in known_otc:
            assert _is_otc(code), f"{code} should be OTC"
    ...
    def test_is_otc_tse_codes(self):
        ...
        for code in tse_codes:
            assert not _is_otc(code), f"{code} should NOT be OTC"
...
# ──────────────────────────────────────────────
# quote_adapter helpers
# ──────────────────────────────────────────────
class TestQuoteAdapterOTC:
    def test_is_otc_known_codes(self):
        from atlas.infrastructure.quote_adapter import _is_otc

        known_otc = ["5269", "6488", "6669", "3293", "8069"]
        for code in known_otc:
            assert _is_otc(code), f"{code} should be OTC"
...
# ──────────────────────────────────────────────
# service_container helpers
# ──────────────────────────────────────────────
class TestServiceContainerOTC:
    def test_is_otc_known_codes(self):
        from atlas.presentation.service_container import _is_otc

        known_otc = ["6669", "5269", "6488", "3293", "8069"]
        for code in known_otc:
            assert _is_otc(code), f"{code} should be OTC"
